    ...
    for i in range(m):
        ...
        for j in range(n):
            if(j+n*i < k):
                ...
                ax[mult*i+0,j].matshow(l[j + n * i][0, :, :], cmap='Greys', )
            else:
...
optimizer_reseau1 = torch.optim.SGD(reseau1.parameters(), lr=learning_rate, momentum=momentum)

#print(evaluate(reseau1, mnist_test))
#train(reseau1, optimizer_reseau1, mnist_train, 1000)
#print(evaluate(reseau1, mnist_test))
...
reseau2 = Reseau2().to(device)
optimizer_reseau2 = torch.optim.Adam(reseau2.parameters(), lr=learning_rate)

print(evaluate(reseau2, mnist_test))
train(reseau2, optimizer_reseau2, mnist_train, 1000)
print(evaluate(reseau2, mnist_test))
